"""
Submittals Page - Highland Tower Development
...
from models.all_models import SubmittalModel
...
# Render header
render_highland_header("📨 Submittals", "Highland Tower Development - Construction Submittals Management")
...
crud_controller = CRUDController(model, 'submittals', display_config)

# Main content tabs
tab1, tab2, tab3 = st.tabs(["📨 Submittals Database", "📝 Create New", "📈 Analytics"])
...
with tab3:
    st.subheader("📈 Submittals Analytics")
    ...
    with col1:
        st.metric("Total Submittals", total_items)
...
# Sidebar
with st.sidebar:
    st.header("Submittals Summary")
    
    items = model.get_all()
    if items:
        st.metric("Highland Tower Submittals", len(items))
    ...
    st.write("Submittals powered by gcPanel")
